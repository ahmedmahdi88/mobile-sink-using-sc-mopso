...
pop(countPop).vel.CH.x=zeros(1,maxN); % maxN because this is adHoc
pop(countPop).vel.CH.y=zeros(1,maxN);
for m=1:MS
    pop(countPop).vel.RVP(m).x=zeros(size(pop(countPop).pos.RVP(m).x));
    pop(countPop).vel.RVP(m).y=zeros(size(pop(countPop).pos.RVP(m).x));
end
pop(countPop).cont=0;
